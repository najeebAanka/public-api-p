...
class AuctionProductController extends BaseController
{
    public function add_new()
    {
        ...
        return view('admin-views.auction-product.add-new', compact('cat', 'br'));
    }
    ...
    public function approve_status(Request $request)
    {
        ...
        return redirect()->route('admin.auction-product.list', ['seller', 'status' => $product['request_status']]);
    }
    ...
    public function deny(Request $request)
    {
        ...
        return redirect()->route('admin.auction-product.list', ['seller', 'status' => 2]);
    }
    ...
    public function view($id)
    {
        ...
        return view('admin-views.auction-product.view', compact('product', 'reviews'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (count($combinations[0]) > 0) {
            foreach ($combinations as $key => $combination) {
                ...
                $item['us'] = $request['us_' . str_replace('.', '_', $str)];
                $item['uk'] = $request['uk_' . str_replace('.', '_', $str)];
                $item['eu'] = $request['eu_' . str_replace('.', '_', $str)];
                $item['sku'] = $request['sku_' . str_replace('.', '_', $str)];
                $item['qty'] = abs($request['qty_' . str_replace('.', '_', $str)]);
                $item['user'] = abs($request['user_' . str_replace('.', '_', $str)]);
        ...
        if ($request->ajax()) {
            ...
            return redirect()->route('admin.auction-product.list', ['in_house']);
        }
    }
    ...
    function list(Request $request, $type)
    {
        ...
        $pro = $pro->where('is_auctions', 1)->orderBy('id', 'DESC')->paginate(Helpers::pagination_limit())->appends(['status' => $request['status']])->appends($query_param);
        return view('admin-views.auction-product.list', compact('pro', 'search', 'request_status'));
    }
    ...
    public function stock_limit_list(Request $request, $type)
    {
        ...
        $pro = $pro->where('is_auctions', 1)->orderBy('id', 'DESC')->paginate(Helpers::pagination_limit())->appends(['status' => $request['status']])->appends($query_param);
        return view('admin-views.auction-product.stock-limit-list', compact('pro', 'search', 'request_status', 'sort_oqrderQty'));
    }
    ...
    public function sku_combination(Request $request)
    {
        ...
        $combinations = Helpers::combinations($options);
        return response()->json([
            'view' => view('admin-views.auction-product.partials._sku_combinations', compact('combinations', 'unit_price', 'colors_active', 'product_name'))->render(),
        ]);
    }
    ...
    public function get_variations(Request $request)
    {
        $product = Product::find($request['id']);
        return response()->json([
            'view' => view('admin-views.auction-product.partials._update_stock', compact('product'))->render()
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin-views.auction-product.edit', compact('categories', 'br', 'product', 'product_category'));
    }
    ...
    {
        return view('admin-views.auction-product.bulk-import');
    }
