...
use App\Resources\AuctionProductBref;
...
class AuctionProductController extends Controller
{
    ...
    {
        $products = ProductManager::get_latest_auction_products($request['limit'], $request['offset']);
        $products['products'] = AuctionProductBref::collection($products['products']);
    ...
    {
        $products = ProductManager::get_top_rated_auction_products($request['limit'], $request['offset']);
        $products['products'] = AuctionProductBref::collection($products['products'], true);
        return response()->json($products, 200);
    }
