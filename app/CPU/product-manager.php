...
class ProductManager
{
    ...
    public static function get_latest_auction_products($limit = 10, $offset = 1)
    {
        $paginator = Product::active()->where('is_auctions', 1)->with(['rating'])->latest()->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
        return [
            'total_size' => $paginator->total(),
            'limit' => (integer)$limit,
    ...
    public static function get_top_rated_auction_products($limit = 10, $offset = 1)
    {
        $reviews = Review::with('product')
            ->whereHas('product', function ($query) {
                $query->where('is_auctions',1)->active();
            })
            ->select('product_id', DB::raw('AVG(rating) as count'))
            ->groupBy('product_id')
            ->orderBy("count", 'desc')
