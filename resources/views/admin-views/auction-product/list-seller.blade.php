                ...
                <div class="card">
                    @if($pro->first() != null && $pro->first()->added_by == 'in_house')
                        <div class="card-header">
                            ...
                            <a href="{{route('admin.auction-product.add-new')}}" class="btn btn-primary  float-right">
                            ...
                            <table id="datatable"
                                ...
                                @foreach($pro as $k=>$p)
                                    <tr>
                                        <th scope="row">{{$k+1}}</th>
                                        <td>
                                            <a href="{{route('admin.auction-product.view',[$p['id']])}}">
                                        ...
                                        <td>
                                            <a class="btn btn-primary btn-sm"
                                               href="{{route('admin.auction-product.edit',[$p['id']])}}">
                                            ...
                                            <form action="{{route('admin.auction-product.delete',[$p['id']])}}"
...
@push('script')
    ...
    <script>
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.auction-product.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
        ...
        function featured_status(id) {
            ...
            $.ajax({
                url: "{{route('admin.auction-product.featured-status')}}",
                method: 'POST',
                data: {
                    id: id
                },
